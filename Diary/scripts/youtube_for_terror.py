...
def df_data(df):
    for i in range(len(df)):
        ...
        if Recommend_Youtube.objects.filter(link_url__iexact=video_url).count() == 0:
            Recommend_Youtube(link_url=video_url,title =video_name, thumnail =video_thumbnails, emotion = Emotion_Category.objects.get(name = "terror"), viewCount = 0).save()
            print("저장완료")
        else : 
            print("중복값")
...
def run():
    ...
    df_data(playlist("두려움을 극복해주는 음악"))
