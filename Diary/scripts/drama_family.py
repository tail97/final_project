...
def drama_famile_func(x):
    drama_famile = []
    iter_num = len(x)
    for i in range(iter_num):
        if ('10751' in str(x[i]['fields']['genres'])) and ('18' in str(x[i]['fields']['genres'])):
            drama_famile.append(x[i]['fields'])
        else:
            continue
    # print(drama_famile)
    print("## 원하는 장르 결과 :  ", drama_famile.__len__())
    return drama_famile
...
def dbSave(drama_famile):
    print(drama_famile)
    for obj in drama_famile:
        except_num = 0 
        # print(obj, type(obj))
        print('-----------------------------')
        try :
...
def run():
    ...
    # print(total_data)
    dbSave(drama_famile_func(total_data))
